...
function AuthScreen({view,setView,toast,onProfileCreated}){
  ...
  const handleRegister=async()=>{
    ...
    setLoading(true);setErr("");
    const{data,error}=await sb.auth.signUp({email,password,options:{data:{full_name:fullName},emailRedirectTo:"https://covoit-allondaz.vercel.app"}});
    if(error){setLoading(false);setErr("Erreur : "+error.message);return;}
    const uid=data?.user?.id;
    if(!uid){setLoading(false);setErr("Erreur d'inscription. Réessayez.");return;}
    let avatar_url=null;
    if(avatarFile){const path=`${uid}/avatar`;await sb.storage.from("avatars").upload(path,avatarFile,{upsert:true});const{data:{publicUrl}}=sb.storage.from("avatars").getPublicUrl(path);avatar_url=publicUrl;}
    await sb.from("profiles").upsert([{id:uid,full_name:fullName,phone,avatar_url}]);
    onProfileCreated({id:uid,full_name:fullName,phone,avatar_url});
    setLoading(false);
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,background:"linear-gradient(160deg,#F2F5EE 0%,#E0EBD5 100%)"}}>
      <div style={{textAlign:"center",marginBottom:28}}>
        ...
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:30,fontWeight:700,color:"#2E5339"}}>Covoit' {VILLAGE}</div>
        <div style={{fontSize:13,color:"#6A8A60",marginTop:6,fontWeight:700}}>Annecy · Albertville · Ugine · Chambéry</div>
      ...
      <div style={{background:"#fff",borderRadius:22,padding:28,width:"100%",maxWidth:420,boxShadow:"0 8px 40px rgba(0,0,0,.12)",border:"1px solid #E2E8D8"}}>
          ...
          {view==="register"&&(
            <>
              <div style={{display:"flex",alignItems:"center",gap:16}}>
                <div onClick={()=>fileRef.current.click()} style={{width:72,height:72,borderRadius:18,background:avatarPreview?"transparent":"#EAF4E0",overflow:"hidden",cursor:"pointer",flexShrink:0,border:"2px dashed #B8D4A0",display:"flex",alignItems:"center",justifyContent:"center"}}>
                ...
                <div>
                  ...
                  <button onClick={()=>fileRef.current.click()} style={{marginTop:6,background:"none",border:"1px solid #C8D5BE",borderRadius:7,padding:"4px 12px",fontSize:12,fontWeight:700,color:"#4A7C59"}}>{avatarPreview?"Changer":"Choisir une photo"}</button>
                  <input ref={fileRef} type="file" accept="image/*" onChange={pickAvatar} style={{display:"none"}}/>
                </div>
              </div>
              <FldA label="Prénom et nom *"><input value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Marie Dupont" style={IA()}/></FldA>
              <FldA label="Téléphone"><input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="06 12 34 56 78" style={IA()}/></FldA>
            </>
          )}
        ...
        <button onClick={view==="login"?handleLogin:handleRegister} disabled={loading} style={{width:"100%",marginTop:20,background:loading?"#9AB89A":"linear-gradient(135deg,#2E5339,#4A7C59)",color:"#fff",border:"none",borderRadius:13,padding:"14px",fontWeight:900,fontSize:16,boxShadow:"0 6px 20px rgba(46,83,57,.3)"}}>
        ...
        <div style={{textAlign:"center",marginTop:14,fontSize:12,color:"#888"}}>
          ...
          <button onClick={()=>{setView(view==="login"?"register":"login");setErr("");}} style={{background:"none",border:"none",color:"#4A7C59",fontWeight:800,fontSize:12,cursor:"pointer",textDecoration:"underline"}}>{view==="login"?"S'inscrire gratuitement":"Se connecter"}</button>
        </div>
      </div>
    </div>
  );
...
function MainApp({session,profile,setProfile,toast}){
  ...
  const loadTrips=useCallback(async()=>{
    ...
    setProfilesMap(map);setLoading(false);
  },[]);
        ...
        <div style={{maxWidth:760,margin:"0 auto",padding:"0 16px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0 12px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <button onClick={()=>sb.auth.signOut()} style={{background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.3)",borderRadius:8,padding:"6px 12px",color:"#fff",fontSize:12,fontWeight:700}}>Déconnexion</button>
            </div>
          </div>
          <div style={{display:"flex",gap:1,background:"rgba(0,0,0,.15)",borderRadius:"10px 10px 0 0",overflow:"hidden"}}>
            {[["search","🔍 Trajets"],["publish","➕ Proposer"],["mine","📋 Mes trajets"],["account","👤 Mon profil"]].map(([key,label])=>(
              <button key={key} onClick={()=>setTab(key)} style={{flex:1,background:tab===key?"rgba(255,255,255,.18)":"transparent",border:"none",color:tab===key?"#fff":"rgba(255,255,255,.55)",fontWeight:tab===key?800:600,fontSize:12,padding:"10px 4px",borderBottom:tab===key?"3px solid #A8D070":"3px solid transparent",transition:"all .2s"}}>{label}</button>
            ))}
          </div>
        </div>
      </header>
          ...
          <div className="fade">
            ...
            {loading?<Spinner inline/>:displayList.length===0?<Empty label="Aucun trajet" sub="Proposez votre aide !"/>:
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                {displayList.map((t,i)=><TripCard key={`${t.id}-${t.occDate}`} trip={t} i={i} driver={profilesMap[t.user_id]} isMine={t.user_id===session.user.id} onContact={()=>setDriverModal({driver:profilesMap[t.user_id],trip:t})}/>)}
              </div>
            }
          </div>
        )}
          ...
          <div className="fade">
            ...
            {myTrips.length===0?<Empty label="Aucun trajet publié" sub={<button onClick={()=>setTab("publish")} style={{background:"none",border:"none",color:"#4A7C59",fontWeight:800,fontSize:13,cursor:"pointer",textDecoration:"underline"}}>Proposer un trajet</button>}/>:
              <div style={{display:"flex",flexDirection:"column",gap:16}}>
                {myTrips.map((t,i)=>{const occ=getOccurrences(t);return(
                  <div key={t.id} style={{background:"#fff",borderRadius:16,border:"1px solid #E2E8D8",overflow:"hidden",boxShadow:"0 2px 14px rgba(0,0,0,.07)"}}>
                    ...
                    {t.recurrence_type!=="none"&&occ.length>0&&(
                      <div style={{padding:"10px 18px 14px",background:"#F8FAF5",borderTop:"1px solid #E2E8D8"}}>
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{occ.slice(0,6).map(d=><span key={d} style={{background:"#EAF4E0",color:"#2E5339",borderRadius:6,padding:"3px 10px",fontSize:12,fontWeight:700}}>{fmtDate(d)}</span>)}</div>
                      </div>
                    )}
                    <div style={{padding:"0 18px 16px"}}><button onClick={()=>handleDelete(t.id)} style={{width:"100%",background:"none",border:"1.5px solid #FECACA",color:"#DC2626",borderRadius:10,padding:"9px",fontWeight:800,fontSize:13}}>Supprimer</button></div>
                  </div>
                );})}
              </div>
            }
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(4px)"}} onClick={()=>setDriverModal(null)}>
          <div className="fade" style={{background:"#fff",borderRadius:22,padding:28,maxWidth:380,width:"100%",boxShadow:"0 30px 80px rgba(0,0,0,.25)"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{background:"#F8FAF5",borderRadius:12,padding:14,marginBottom:18,border:"1px solid #E2E8D8"}}>
              <div style={{display:"flex",gap:6,marginBottom:6}}><span>📍</span><span style={{fontSize:13,fontWeight:600}}>{driverModal.trip.from_place}</span></div>
              <div style={{display:"flex",gap:6,marginBottom:8}}><span>🏁</span><span style={{fontSize:13}}><strong style={{color:"#2E5339"}}>{driverModal.trip.to_city}</strong> — {driverModal.trip.to_address}</span></div>
            ...
            {driverModal.driver?.phone?<a href={`tel:${driverModal.driver.phone.replace(/\s/g,"")}`} style={{display:"block",textAlign:"center",background:"linear-gradient(135deg,#2E5339,#4A7C59)",color:"#fff",borderRadius:12,padding:"13px",fontWeight:900,fontSize:16,textDecoration:"none",marginBottom:10}}>📞 Appeler le {driverModal.driver.phone}</a>:<div style={{background:"#FEF3C7",borderRadius:10,padding:"12px",color:"#92400E",fontSize:13,fontWeight:700,textAlign:"center",marginBottom:10}}>Pas de téléphone renseigné.</div>}
      ...
      <div style={{background:"#fff",borderRadius:18,padding:24,boxShadow:"0 2px 16px rgba(0,0,0,.07)",border:"1px solid #E2E8D8"}}>
        ...
        <Sec title="Destination">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <Fld label="Adresse *" err={errs.toAddress}><input value={form.toAddress} onChange={e=>f("toAddress",e.target.value)} placeholder="Gare, marché…" style={IS(errs.toAddress)}/></Fld>
          </div>
        </Sec>
        ...
        <Sec title="Infos">
          <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:12}}>
            <Fld label="Places"><select value={form.seats} onChange={e=>f("seats",e.target.value)} style={IS()}>{[1,2,3,4,5,6].map(n=><option key={n} value={n}>{n} place{n>1?"s":""}</option>)}</select></Fld>
            <Fld label="Message"><textarea value={form.note} onChange={e=>f("note",e.target.value)} placeholder="Infos utiles…" rows={2} style={IS()}/></Fld>
          </div>
        </Sec>
    ...
    <div className="fade">
      <div style={{background:"#fff",borderRadius:18,padding:24,boxShadow:"0 2px 16px rgba(0,0,0,.07)",border:"1px solid #E2E8D8"}}>
        ...
        <Sec title="Mes informations">
          <Fld label="Prénom et nom *"><input value={name} onChange={e=>setName(e.target.value)} style={IS()}/></Fld>
          <Fld label="Téléphone"><input value={phone} onChange={e=>setPhone(e.target.value)} style={IS()}/></Fld>
        </Sec>
...
function TripCard({trip,i,driver,isMine,onContact}){
  ...
  return(
    <div className={isMine?"":"lift"} style={{background:"#fff",borderRadius:14,padding:18,border:"1px solid #E2E8D8",boxShadow:"0 2px 12px rgba(0,0,0,.06)"}}>
      ...
      {!isMine&&onContact&&<button onClick={onContact} style={{width:"100%",marginTop:14,background:"#2E5339",color:"#fff",border:"none",borderRadius:10,padding:"10px",fontWeight:800,fontSize:14}}>Voir le profil & contacter →</button>}
    </div>
  );
}
